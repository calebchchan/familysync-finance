...
export default function PoolsView() {
  ...
  return (
    <div className="pb-4">
      ...
      <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-4 shadow-sm border border-amber-100 mb-5">
        <p className="text-xs text-amber-700 font-medium mb-1">Total Joint Value</p>
        <p className="text-2xl font-bold text-amber-800">{fmtMoney(totalJointValue)}</p>
        <p className="text-xs text-amber-600 mt-1">{pools.length} active pools</p>
      </div>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-gray-900">{pool.name}</span>
                </div>
                <span className="text-xs text-gray-400">Target: {fmtMoney(pool.target)}</span>
              </div>

              <div className="grid grid-cols-3 gap-2 mb-3">
                <div>
                  <p className="text-[10px] text-gray-400 uppercase">Pledged</p>
                  <p className="text-sm font-semibold text-indigo-600">{fmtMoney(totalPledged)}</p>
                </div>
                <div>
                  <p className="text-[10px] text-gray-400 uppercase">Spent</p>
                  <p className="text-sm font-semibold text-red-500">{fmtMoney(totalSpent)}</p>
                </div>
                <div>
                  <p className="text-[10px] text-gray-400 uppercase">Available</p>
                  <p className="text-sm font-semibold text-emerald-600">{fmtMoney(available)}</p>
                </div>
              </div>
...
function PoolDetail({
  ...
  return (
    <Modal open onClose={onClose} title={`${pool.icon} ${pool.name}`}>
      {/* Pledges */}
      <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Pledges</h3>
      <div className="space-y-2 mb-4">
        {poolPledges.length === 0 && <p className="text-sm text-gray-400">No pledges yet</p>}
        {poolPledges.map((p) => (
          <div key={p.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2">
            <div>
              <span className="text-sm font-medium text-gray-900">{getUserName(p.userId)}</span>
              {p.recurring && (
            ...
            <span className="text-sm font-semibold text-indigo-600">{fmtMoney(p.amount)}</span>
          </div>
        ))}
      </div>
      ...
      <button
        onClick={onPledge}
        className="w-full py-2 rounded-lg text-sm font-medium bg-indigo-50 text-indigo-600 hover:bg-indigo-100 mb-4"
      >
        + Add Pledge
      </button>
      ...
      <div className="space-y-2">
        {poolTxs.length === 0 && <p className="text-sm text-gray-400">No transactions yet</p>}
        {poolTxs.map((t) => (
        ...
        <div className="flex items-center gap-2">
          ...
          {recurring && (
            <select
              ...
              className="ml-auto border border-gray-200 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
